...
# Padding Collator
# [DataCollatorForLanguageModeling](https://github.com/huggingface/transformers/blob/main/src/transformers/data/data_collator.py#L402) をベースに実装。
# 正しいトークンで Padding を行うために Tokenizer が必要なため、tokenizer_name を SageMaker の Hyperparameter で渡す必要がある。
class PaddingCollator:
    
    def __init__(self, tokenizer):
        self.tokenizer = tokenizer
        
    def __call__(self, examples: Tuple[torch.Tensor, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        # 入力は iids, attns の Tuple[torch.Tensor, torch.Tensor] なので 
        # Tokenizer が Pad できるように List[Dict[str, any]] に変換
        examples = [{"input_ids": x[0], "attention_mask": x[1]} for x in examples]
        # Padding の付与
        batch = self.tokenizer.pad(examples, return_tensors="pt")
        # Padding を学習しないように labels を -100 にする
        labels = batch["input_ids"].clone()
        if self.tokenizer.pad_token_id is not None:
            labels[labels == self.tokenizer.pad_token_id] = -100
        batch["labels"] = labels
        # 受け取り側は Map ではなく Tuple で受け取るため変換
