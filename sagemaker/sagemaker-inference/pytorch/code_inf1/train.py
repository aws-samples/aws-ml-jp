...
def transform_model_for_neo(model, input_shape):
    model = torch.jit.trace(model.float().eval(), torch.zeros(input_shape).float())
    return model
...
def train(args):
    ...
    save_model(model, args.model_dir, input_shape)
...
def save_model(model, model_dir, input_shape):
    logger.info("Saving the model.")
    model = transform_model_for_neo(model, input_shape)
